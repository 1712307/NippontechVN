...
$('html').on('click','.change-click .item',function() {
	var change = $(this).attr('data-change');
	$('html').find('.list-3 li').removeClass('show-img');
	$('html').find('.list-3 li#' + change).addClass('show-img');
});

$('html').on('click','.color-click .item',function() {
	var color = $(this).attr('data-color');
	$('html').find('.slides2 ul li').removeClass('hienra');
	$('html').find('.slides2 ul li#' + color).addClass('hienra');
});
...
document.addEventListener("DOMContentLoaded", function() {
	...
	var bg_ani = document.querySelectorAll('.bg-ani');
	var text_show = document.querySelectorAll('.move-text p');
	...
	chuyenslide = function() {
		...
		for(var i = 0; i < slides.length; i++){
			...
			text_big[i].classList.remove('style-ani-1');
			text_show[i].classList.remove('style-text-ani');
			bg_ani[i].classList.remove('style-bg-ani');

		}
		if(vitrislideht == 0){
			vitrislideht = cuoicung + 1;
			slides[vitrislideht-1].classList.add('hienlen');
			text_show[vitrislideht-1].classList.add('style-text-ani');
			bg_ani[vitrislideht-1].classList.add('style-bg-ani');
			text_big[vitrislideht-1].classList.add('style-ani-1');
...
// Tu dong chuyen slide
	...
	nutphai.addEventListener('click',chuyenslidep);
                    /* AUTO - SLIDE - 2 */
                      auto2();
        var slides2 = document.querySelectorAll('div.slides2 ul li')
        var cuoicung2 = slides2.length - 1;

    function auto2(){
    var thoigian2 = setInterval(function(){

        var vitrislideht = 0;
        var slideht2 = document.querySelector('ul li.hienra');
        for(var i = 0; slideht2 = slideht2.previousElementSibling; vitrislideht++){

        }
        for(var i = 0; i < slides2.length; i++){
            slides2[i].classList.remove('hienra');
        }
        if(vitrislideht == cuoicung2){
            vitrislideht = -1;

        }

        slides2[vitrislideht+1].classList.add('hienra');

    },3000)
        var stop = document.querySelectorAll('.color-click .item');
        for(var i = 0; i < stop.length; i++){
        stop[i].addEventListener('click',function () {
            clearInterval(thoigian2);
